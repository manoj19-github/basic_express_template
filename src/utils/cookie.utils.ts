...
export class CookieUtils {
  ...
  static setAuthCookies(
    response: Response,
    tokens: { accessToken: string; refreshToken: string, deviceId: string,refreshTokenExpires: Date, accessTokenExpires: Date }
  ): void {
    ...
    response.cookie('token_info', JSON.stringify({
      accessTokenExpires:  tokens.accessTokenExpires.toUTCString(),
      refreshTokenExpires: tokens.refreshTokenExpires.toUTCString(),
    }), {
      ...baseOptions,
      ...
      maxAge: 7 * 24 * 60 * 60 * 1000,
		...
		response.set('Access-Token-Expiry-UTC', tokens.accessTokenExpires.toUTCString());
		response.set('Refresh-Token-Expiry-UTC', tokens.refreshTokenExpires.toUTCString());
